...
class FlashCardApp:
    ...
    def setup_settings_panel(self):
        ...
        self.settings_panel_rect = pygame.Rect(panel_x, panel_y, self.PANEL_WIDTH, self.PANEL_HEIGHT)
    ...
    def setup_ui_elements(self):
        ...
        # Min slider
        self.slider_min_rect = pygame.Rect(panel_x, panel_y + 40, self.SLIDER_WIDTH, self.SLIDER_HEIGHT)
        self.slider_min_handle = pygame.Rect(panel_x, panel_y + 36, 8, 16)
        self.slider_min_value = 4
        
        # Max slider  
        self.slider_max_rect = pygame.Rect(panel_x + 180, panel_y + 40, self.SLIDER_WIDTH, self.SLIDER_HEIGHT)
    ...
    def draw_settings_panel(self):
        ...
        pygame.draw.rect(panel_surface, self.SETTINGS_BG, (0, 0, self.PANEL_WIDTH, self.PANEL_HEIGHT))
        pygame.draw.rect(panel_surface, self.SETTINGS_BORDER, (0, 0, self.PANEL_WIDTH, self.PANEL_HEIGHT), 2)
        ...
        close_y = 15
        pygame.draw.line(panel_surface, self.BLACK, (close_x, close_y), (close_x + 10, close_y + 10), 2)
        pygame.draw.line(panel_surface, self.BLACK, (close_x + 10, close_y), (close_x, close_y + 10), 2)
        
        # Draw settings content
        self.draw_settings_content(panel_surface)
        
        # Blit the panel to the main window
        self.window.blit(panel_surface, (self.settings_panel_rect.x, self.settings_panel_rect.y))
    ...
    def draw_settings_content(self, surface):
        ...
        panel_offset_x = self.settings_panel_rect.x
        panel_offset_y = self.settings_panel_rect.y
        
        slider_y = y_pos + 25
        # Min slider
        slider_rect = pygame.Rect(margin, slider_y, self.SLIDER_WIDTH, self.SLIDER_HEIGHT)
        ...
        min_text = self.settings_text_font.render(f"Min: {self.slider_min_value}", True, self.BLACK)
        surface.blit(min_text, (margin, slider_y - 20))
        
        # Max slider
        max_slider_x = margin + 180
        slider_rect = pygame.Rect(max_slider_x, slider_y, self.SLIDER_WIDTH, self.SLIDER_HEIGHT)
        handle_rect = pygame.Rect(
        ...
        max_text = self.settings_text_font.render(f"Max: {self.slider_max_value}", True, self.BLACK)
        surface.blit(max_text, (max_slider_x, slider_y - 20))
        
        # Starting Letters section
        y_pos += 70
        label_text = self.settings_label_font.render("Starting Letters:", True, self.BLACK)
        surface.blit(label_text, (margin, y_pos))
        ...
        for i, (_, letter) in enumerate(self.toggle_boxes):
            ...
            letter_text = self.settings_text_font.render(letter.upper(), True, self.BLACK)
        ...
        if self.hyphenate_enabled:
            # Draw checkmark
            pygame.draw.line(surface, self.BLACK, 
                           (checkbox_rect.x + 3, checkbox_rect.y + 9),
                           (checkbox_rect.x + 7, checkbox_rect.y + 13), 2)
            pygame.draw.line(surface, self.BLACK,
                           (checkbox_rect.x + 7, checkbox_rect.y + 13),
                           (checkbox_rect.x + 15, checkbox_rect.y + 5), 2)
        
        hyphen_text = self.settings_text_font.render("Spanish Hyphenation", True, self.BLACK)
    ...
    def handle_settings_panel_click(self, pos):
        ...
        panel_pos = (pos[0] - self.settings_panel_rect.x, pos[1] - self.settings_panel_rect.y)
    ...
    def handle_settings_interactions(self, panel_pos):
        ...
        checkbox_rect = pygame.Rect(margin, y_pos, 18, 18)  # Same as in draw_settings_content
    ...
    def handle_slider_interaction(self, pos, dragging_min, dragging_max):
        ...
        # Min slider handle
        min_handle_x = self.settings_panel_rect.x + self.PANEL_MARGIN + (self.slider_min_value - 4) * self.SLIDER_WIDTH // 4
        min_handle_rect = pygame.Rect(min_handle_x, slider_y - 4, 8, 16)
        
        # Max slider handle  
        max_handle_x = self.settings_panel_rect.x + self.PANEL_MARGIN + 180 + (self.slider_max_value - 4) * self.SLIDER_WIDTH // 4
    ...
    def run(self):
        ...
        while True:
            ...
            if hasattr(self, '_last_cogwheel_hovered') and self._last_cogwheel_hovered != self.cogwheel_hovered:
                self.display_word(current_word)
