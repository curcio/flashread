...
from .utils import setup_nltk, setup_matplotlib
...
class TextProcessor:
    ...
    def process_file(self, filename):
        ...
        try:
            ...
            print(f"Warning: Could not decode file {filename}. Trying with different encoding.")
            try:
    ...
    def process_all_corpus_files(self):
        ...
        print(f"Processed {processed_files} files with {len(self.word_frequencies)} unique words.")
    ...
    def load_word_frequencies(self, filename='word_frequencies.csv'):
        ...
        try:
            ...
            print(f"Loaded {len(self.word_frequencies)} word frequencies from {filepath}")
            return True
        except FileNotFoundError:
    ...
    def plot_word_frequencies(self, top_n=20, save_plot=True):
        ...
        if save_plot:
            plot_path = os.path.join(self.data_dir, 'plots', f'word_frequencies_top_{top_n}.png')
    ...
    def create_vocabulary_dataframe(self, min_frequency=3):
        ...
        print(f"Created vocabulary with {len(self.vocabulary_df)} words (min frequency: {min_frequency})")
    ...
    def load_vocabulary(self, filename='vocabulary.csv'):
        ...
        try:
            ...
            print(f"Loaded vocabulary with {len(self.vocabulary_df)} words from {filepath}")
            return True
        except FileNotFoundError:
