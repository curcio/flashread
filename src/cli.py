  ...
  flashread run --vocabulary custom # Use custom vocabulary file
    ...
    def setup_parser(self):
        ...
        generate_parser = subparsers.add_parser(
            'generate', 
            help='Process corpus files and generate vocabulary data'
        )
        ...
        generate_parser.add_argument(
            '--plot',
            action='store_true',
            help='Generate and show word frequency plot'
        )
        generate_parser.add_argument(
            '--force',
            action='store_true',
        ...
        run_parser = subparsers.add_parser(
            'run',
            help='Launch the flashcard interface'
        )
    ...
    def command_generate(self, args):
        ...
        if not args.force and os.path.exists(word_freq_file) and os.path.exists(vocab_file):
        ...
        vocabulary_df = processor.create_vocabulary_dataframe(min_frequency=args.min_frequency)
        ...
        print(f"   Frequency range: {stats['min_frequency']}-{stats['max_frequency']} occurrences")
